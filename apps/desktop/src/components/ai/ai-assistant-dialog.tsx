  ...
  return (
    <Dialog.Root open={open} onOpenChange={onOpenChange}>
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed right-4 top-16 bottom-16 z-50 flex w-[480px] flex-col rounded-xl border border-(--color-border) bg-(--color-card) shadow-2xl focus:outline-none">
          {/* Header */}
          <div className="flex items-center justify-between border-b border-(--color-border) px-4 py-3">
            <Dialog.Title className="flex items-center gap-2 text-sm font-semibold text-(--color-text-primary)">
              <Sparkles className="h-4 w-4 text-purple-400" />
              AI Assistant
              <span className="rounded bg-(--color-surface) px-1.5 py-0.5 text-[9px] font-medium text-(--color-text-dimmed) uppercase tracking-wider">
                Optional
              </span>
            </Dialog.Title>
            <div className="flex items-center gap-1">
              <button
                ...
                className={`rounded-lg p-1.5 transition-colors hover:bg-(--color-elevated) ${
                  showConfig ? "text-purple-400" : "text-(--color-text-muted)"
                }`}
                title="Settings"
              >
              ...
              <button
                onClick={() => setMessages([])}
                className="rounded-lg p-1.5 text-(--color-text-muted) transition-colors hover:bg-(--color-elevated)"
                title="Clear chat"
              >
                <Trash2 className="h-3.5 w-3.5" />
              </button>
              <Dialog.Close className="rounded-lg p-1.5 text-(--color-text-muted) transition-colors hover:bg-(--color-elevated)">
          ...
          {/* Config panel */}
          {showConfig && (
            <div className="border-b border-(--color-border) bg-(--color-surface) px-4 py-3 text-xs">
              ...
              <p className="text-(--color-text-dimmed)">
              ...
              {isConfigured && (
                <div className="mt-2 flex gap-3 text-(--color-text-dimmed)">
                  <span>Model: <code className="text-(--color-text-secondary)">{aiModel}</code></span>
                  <span>Endpoint: <code className="text-(--color-text-secondary)">{aiEndpoint?.replace("https://", "")}</code></span>
                </div>
              )}
            </div>
          )}
          ...
          {!isConfigured && (
            <div className="flex flex-1 flex-col items-center justify-center gap-4 px-8 py-10 text-center">
              ...
              <div className="w-full rounded-lg border border-(--color-border) bg-(--color-surface) p-4 text-left text-xs text-(--color-text-muted) space-y-2">
                <p className="font-medium text-(--color-text-secondary)">To get started:</p>
                ...
                <p className="pt-1 text-(--color-text-dimmed)">
              ...
              <p className="text-[10px] text-(--color-text-dimmed)">
          ...
          {isConfigured && (
            <>
              ...
              <div className="border-b border-(--color-border) bg-amber-500/5 px-4 py-2 text-[10px] text-(--color-text-dimmed)">
              ...
              <div ref={logRef} className="flex-1 overflow-auto px-4 py-3 space-y-3">
                {messages.length === 0 && (
                  <div className="flex flex-col items-center gap-2 py-8 text-center text-xs text-(--color-text-dimmed)">
              ...
              {/* Quick actions */}
              <div className="flex gap-1.5 border-t border-(--color-border) px-4 py-2">
                <button
                  onClick={handleGenerateTests}
                  disabled={loading || !tab?.response}
                  className="flex items-center gap-1.5 rounded-lg bg-(--color-elevated) px-2.5 py-1.5 text-[10px] font-medium text-(--color-text-secondary) hover:bg-(--color-border) disabled:opacity-40"
                >
              ...
              <div className="border-t border-(--color-border) px-4 py-3">
                <div className="flex items-end gap-2">
                  <textarea
                    ...
                    disabled={loading}
                    className="flex-1 resize-none rounded-lg bg-(--color-elevated) px-3 py-2 text-sm text-(--color-text-primary) placeholder-(--color-text-dimmed) outline-none focus:ring-1 focus:ring-purple-500 disabled:opacity-50"
                    rows={2}
                    onKeyDown={(e) => {
    ...
    <div className={`flex flex-col ${isUser ? "items-end" : "items-start"}`}>
      <div
        className={`max-w-[90%] rounded-xl px-3 py-2 text-sm ${
          isUser
            ...
            : message.error
              ? "bg-red-500/10 text-(--color-text-primary) border border-red-500/20"
      ...
      >
        ...
        {message.runPrompt && (
          <div className="mt-2 flex gap-1.5">
            ...
            <button
              ...
              className="flex-1 rounded-lg bg-(--color-elevated) px-2 py-1.5 text-xs font-medium text-(--color-text-secondary) hover:bg-(--color-border)"
            >
              I'll run it myself
            </button>
          </div>
        ...
        {message.generatedRequest && (
          <div className="mt-2 rounded-lg border border-(--color-border) bg-(--color-surface) p-2">
            ...
            {message.generatedRequest.body && (
              <pre className="mt-1 max-h-24 overflow-auto whitespace-pre-wrap text-[10px] text-(--color-text-muted) font-mono">
        ...
        {message.generatedTests && (
          <div className="mt-2 rounded-lg border border-(--color-border) bg-(--color-surface) p-2">
            <pre className="max-h-32 overflow-auto whitespace-pre-wrap text-[10px] text-(--color-text-secondary) font-mono">
            ...
            <div className="mt-2 flex gap-1.5">
              ...
              <button
                ...
                className="rounded-lg bg-(--color-elevated) px-2 py-1.5 text-xs text-(--color-text-secondary) hover:bg-(--color-border)"
              >
      ...
      {message.debug && (
        <button
          ...
          className="mt-0.5 flex items-center gap-1 text-[10px] text-(--color-text-dimmed) hover:text-(--color-text-muted)"
        >
      ...
      {message.debug && showDebug && (
        <div className="mt-0.5 max-w-[90%] rounded bg-(--color-surface) p-2 text-[10px] text-(--color-text-dimmed) font-mono">
      ...
      {/* Timestamp */}
      <span className="mt-0.5 text-[9px] text-(--color-text-dimmed)">
